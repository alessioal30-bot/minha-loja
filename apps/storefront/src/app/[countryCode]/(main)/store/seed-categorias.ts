...
export default async function seedCategorias({ container }: { container: MedusaContainer }) {
  ...
  // Lógica para injetar no Medusa (via Product Module Service)
  ...
  for (const catData of categoriasPrincipal) {
    ...
    if (existingParent.length > 0) {
      ...
      logger.info(`Categoria pai já existe: ${parentData.name}`)
    } else {
    ...
    if (category_children && category_children.length > 0) {
      for (const childData of category_children) {
        ...
        if (existingChild.length === 0) {
          ...
          logger.info(` -> Subcategoria já existe: ${childData.name}`)
        }
      }
    }
  }

  logger.info("Mapeamento de categorias concluído com sucesso!")
}