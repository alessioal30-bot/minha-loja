...
const OptionsPicker = ({
  ...
  return (
    <div className="flex flex-col gap-y-4">
      ...
      >
        {options.map((option) => {
          const rawValues = (option as any).values || []
          const values = rawValues
            .map((val: any) => ({
              id: val.id,
              label: typeof val.value === "string" ? val.value : (val.value?.value || String(val.id || "")),
            }))
            .filter((val: { id: string; label: string }) => !!val.id && !!val.label)
          ...
          const toggleValue = (valueId: string) => {
            ...
            const nextSelections = isSelected
              ? selectedValueIds.includes(valueId) ? selectedValueIds.filter((id) => id !== valueId) : selectedValueIds
          ...
          const selectedCount = values.filter((value: { id: string; label: string }) =>
